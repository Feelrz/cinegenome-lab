(() => {
  ...
  function directorFingerprint(movie) {
    ...
    const peers=MOVIES.filter(m => director && director.toLowerCase()!=='unknown' && String(m.director||'').trim().toLowerCase()===director.toLowerCase() && m.dna);
    ...
    return { dna, count:source.length, label:director||'Unknown' };
  }
